...
- **请使用中文回复**
- **write 工具一次不要写入太长的数据，建议 100 行左右写入一次**
...
**规格书**：`docs/系统设计规格书.md`（所有功能需求的权威来源）

**开发规范文档**（开发前必读）：

| 文档 | 路径 | 说明 |
| ---- | ---- | ---- |
| API 接口规范 | `docs/specs/api-contracts.md` | 所有 REST 端点定义、请求/响应格式 |
| 认证集成规范 | `docs/specs/auth-integration.md` | JWT 存储策略、路由保护、Token 刷新拦截器 |
| 前端设计规范 | `docs/specs/frontend-design-system.md` | 设计令牌、Tailwind 配置、组件选型、路由树 |
| 状态管理规范 | `docs/specs/state-management.md` | Zustand Store、SWR 配置、React Hook Form 约定 |
| 错误码规范 | `docs/specs/error-codes.md` | 全模块错误码枚举、HTTP 状态码映射 |
| 开发路线图 | `docs/specs/development-roadmap.md` | 阶段划分、模块依赖顺序、可复用资产清单 |
